...
6. `startor setup` installs tor through Homebrew, and installs Homebrew first where it is missing. That second half no longer works: it fetches `https://raw.githubusercontent.com/Homebrew/install/master/install`, which now answers 404, so nothing is run and the `brew install tor` after it fails for want of brew. On a machine which already has Homebrew — which is to say every machine this has been run on — only the tor half is reached, and that works. Install [Homebrew](https://brew.sh) yourself first.
